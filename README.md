...
HTML::StateTable::ResultSet::File - Iterator pattern for viewing files

# Synopsis

    use HTML::StateTable::ResultSet::File;
...
- complete
    ...
    A mutable boolean which is true if the results list contains all the lines
    in the file. It is false when the results list only contains a partial
    view of the file
...
- distinct\_column
    ...
    The column filter option in [HTML::StateTable](https://metacpan.org/pod/HTML%3A%3AStateTable) will specify which column to
    return unique values for

- has\_distinct\_column
...
- get\_column( column\_name )
    ...
    Returns a [HTML::StateTable::ResultSet::File::Column](https://metacpan.org/pod/HTML%3A%3AStateTable%3A%3AResultSet%3A%3AFile%3A%3AColumn) object for the given
    column name
